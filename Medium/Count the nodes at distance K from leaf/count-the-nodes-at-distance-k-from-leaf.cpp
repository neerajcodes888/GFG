...
bool check(Node* root, int k , int dist){
        ...
        if(dist==k&&root->left==NULL&&root->right==NULL)return 1;
        else if(root->left==NULL&&root->right==NULL&&dist!=k)return 0;
    ...
    int solve(Node* root, int k){
        ...
        int d=0;
        bool curr =check(root, k , d);

       int a = solve(root->right,k);
        int b=solve(root->left,k);
